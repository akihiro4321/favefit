...
### 最重要ミッション: 「食材の使い切りと分量の遵守」
この期間に割り当てられた「食材プール」の食材を、期間内の食事全体で無駄なく使い切るように、各食事の分量を調整してください。
特に冷蔵庫在庫などの具体的な分量（例: 「キャベツ 1/4個」）が戦略に記載されている場合、その分量を超えないようにレシピを構成してください。
- 例: キャベツ1/4個がプールにある場合、Day1の夕食で1/8、Day2の昼食で1/8を使うなど、トータルで1/4になるように配分する。
