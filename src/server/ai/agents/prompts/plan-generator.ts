...
- **adaptiveDirective (適応型プランニング指示 - 最優先)**:
  ユーザーの「現状の食生活」に基づいた具体的な調整指示が含まれています。
  1. \`baseCalories\` が指定されている場合、計算上の目標値 (\`targetCalories\`) ではなく、この値を1日の基準として優先してください。
  2. \`instructions\` リストに含まれる指示（例：「無理のない範囲で増やす」など）は、数値目標の厳密な達成よりも優先してください。
  3. **重要**: 数値の帳尻合わせのために、人間が食べ切れないような大量の食事（例: 卵5個、ご飯500gなど）を提案することは絶対に避けてください。
