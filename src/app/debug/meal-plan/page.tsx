...
export default function DebugMealPlanPage() {
  ...
  const runSingleScenario = async (id: string) => {
    ...
    setScenarios((prev) =>
      prev.map((s) =>
        s.id === id ? { ...s, isLoading: true, error: null, result: null } : s
      )
    );
    ...
    try {
      ...
      setScenarios((prev) =>
        prev.map((s) =>
          s.id === id ? { ...s, result: data, isLoading: false } : s
        )
      );
    } catch (err) {
      setScenarios((prev) =>
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 lg:col-span-3 space-y-4">
          ...
          <Textarea
            ...
            onChange={(e) =>
              updateScenarioJson(activeScenario.id, e.target.value)
            }
              ...
              <Tabs defaultValue="preview" className="w-full">
                <div className="flex items-center justify-between mb-6 sticky top-0 bg-transparent py-2 backdrop-blur-sm z-10">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      {activeScenario.result.debug?.executionTimeMs}ms
                    </Badge>
                  </div>
                </div>

                <TabsContent value="preview" className="space-y-8 m-0">
                  {Object.entries(activeScenario.result.days).map(
                    ([date, day]: [string, DayPlan]) => (
                      <div key={date} className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
                          {Object.entries(day.meals).map(
                            ([type, meal]: [string, MealSlot]) => (
                              <div
                                key={type}
                                className="bg-background border-2 border-border/50 rounded-xl p-3 shadow-sm hover:border-primary/30 transition-all group relative"
                              >
                                  ...
                                  <div className="flex flex-wrap gap-1">
                                    {meal.tags
                                      ...
                                      .map((tag: string) => (
                                        <span
                                          key={tag}
                                          className="text-[8px] bg-secondary/30 text-secondary-foreground px-1.5 py-0.5 rounded-md"
                                        >
                                          {tag}
                                        </span>
                                      ))}
