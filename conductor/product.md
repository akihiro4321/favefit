...
## 1. プロジェクト概要
**FaveFit**（Favorite + Fit）は、従来の「制限」と「記録」というダイエットのアプローチを逆転させた、ダイエット支援アプリです。カロリーを計算する代わりに、ユーザーが「今食べたいもの」や「好きな食材」を入力すると、パーソナライズされたAIエージェントが目標に合わせた栄養バランスの良いレシピを提案します。

## 2. ターゲットユーザーと利用シーン
### ターゲット層
- ある程度の自炊経験があり、見た目の改善（ダイエット）を目指している方。
- 食事制限や面倒な食事記録で過去に挫折した経験がある方。

### 主な利用シーン
- **朝の献立作成:** その日の食事プランを立てる時。
- **買い物中:** スーパーで良さそうな食材を見つけ、何を作るか決める時。
- **通勤・帰宅中:** 仕事の合間に、パパッと作れる健康的なアイデアを探す時。

## 3. コアバリュー（核となる価値）
- **我慢ではなく「楽しみ」を:** 「食べてはいけないもの」ではなく「食べたいもの」にフォーカスします。
- **思考の自動化:** 面倒な栄養計算はAIが行い、ユーザーは気分を選ぶだけで済みます。
- **深いパーソナライズ:** フィードバックを通じてシステムが学習し、時間とともに最高のパートナーへと進化します。

## 4. ユーザーが目指すゴール
- **自信の獲得:** 数値だけでなく、鏡に映る自分の姿や健康状態に自信を持てるようになる。
- **楽しさの発見:** ダイエットが「苦行」から「料理の探求」へと変わる。
- **習慣の定着:** 健康的で経済的な自炊が、無理なく自然なライフスタイルとして定着する。

## 5. AIのキャラクターと対話スタイル
- **役割:** 親しみやすく、共感してくれる「相棒（パートナー）」。
- **トーン:** 励まし上手で、決して否定せず、食べることを一緒に楽しむ姿勢。
- **関係性:** ユーザーの好みや苦労を理解し、ともに成長していくパートナー。

## 6. 競合との差別化ポイント
- **食事の幸福感:** 好きな食材を起点にすることで、ダイエット中とは思えない満足度を提供します。
- **進化する関係性:** 静的なデータベースではなく、使うほどにユーザーの好みを深く理解し、提案が洗練されていく「絆」を構築します。
